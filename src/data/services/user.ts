...
export class UserService implements IUserService {
  ...
  async validateCreateUser (dataForm: TUser): Promise<void> {
    ...
    if (result.length > 0) {
      await createError([
        createObjectError(['email'], 'E-mail informado já existe!')
      ])
    }
  ...
  async create (dataForm: TUser): Promise<TUserCreate> {
    ...
    dataForm.password = await BCrypt.createPasswordHash(
      dataForm.password,
      dataForm.password_confirmation
    )
    delete dataForm.password_confirmation

    const { name, email, active, level } = await this.userRepository.toCreate(
      dataForm
    )
