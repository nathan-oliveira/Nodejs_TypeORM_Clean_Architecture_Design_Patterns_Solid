...
import { errorCreatePassword, errorPasswordConfirmation, errorComparePassword, errorCompareUser } from './errors'
...
export class BCrypt {
  static async createPasswordHash (password: string, passwordConfirmation: string): Promise<string> {
    ...
    if (password !== passwordConfirmation) await createError(errorPasswordConfirmation)
